class DTreeNode {
	...
	private char mUniformVal; // If this node contains uniform data (is a leaf),
							  // this will be the uniform label (PARTY_D or PARTY_R).
	private double mEntropy;
	private char mVoteType; // The vote type that this node splits on, either VOTE_YAY, VOTE_NAY, VOTE_PRESENT or VOTE_NA
	private int mSplitOnVote; // The index of the vote on which this node splits
	private DTreeNode[] mChildren;
	private DTreeNode mParent; // In case we need to get parent's majority
	private Representative[] mData;
	private char mMajorityParty; // The majority party at this leaf, or PARTY_TIE if tied
	 ...
	 * @param voteType The vote type that this node represents, either NAY, YAY or PRESENT
	 */
	...
	/**
	 * Checks if this node is uniform. If so, it returns the uniform label (PARTY_D or PARTY_R)
	...
	private char checkUniformity() {
		...
		for(Representative r : mData) {
			...
			// If there exists D and R labels in the node's data set, node is not uniform
		...
		if(dCount > 0) {
			...
			// This means we have an empty data list. This node will become a leaf
			// whose uniform value is the majority party of the parent node (or random if no parent)
			party = mParent == null ? DecisionTree.randomParty() : mParent.getMajorityParty();
		}
		return party;
	}
	 ...
	 * which has a majority. Finally, if we've recursed up the tree to the root and
	 * still haven't found a majority, the tie is broken randomly
	...
	/**
	 * Recursive helper for finding majority. This method is "wrapped" with getMajorityParty()
	 * to make the API cleaner. It would be awkward for callers to call this method on a
	 * DTreeNode instance, and then require them to specify that instance as an argument
	...
	public boolean isUniform() {
		if (mUniformVal == DecisionTree.PARTY_D || mUniformVal == DecisionTree.PARTY_R)
			return true;
		else
			return false;
	}
	...
	public String toString() {
		// We can take advantage of letter binary representations to convert from 
		// issue number to corresponding issue letter. 'A' + 1 is B, 'A' + 2 is C, etc.
		char a = 'A'; //
		String str = "";
		if(mUniformVal == DecisionTree.NOT_UNIFORM)
			str = (mVoteType == DecisionTree.VOTE_NA ? " " : mVoteType) + " Issue " + (char)(a + mSplitOnVote);
		else 
